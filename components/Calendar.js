...
import { Fugaz_One, Mountains_of_Christmas } from 'next/font/google';
...
export default function Calendar(props) {
  ...
  const [selectedMonth, setSelectedMonth] = useState(monthsArr[curMonth]); 
  ...
  const monthNow = new Date(selectedYear, monthsArr.indexOf(selectedMonth));
  const firstDayOfMonth = monthNow.getDay();
  const daysInMonth = new Date(selectedYear, monthsArr.indexOf(selectedMonth) + 1, 0).getDate();

  console.log(`Days in ${selectedMonth} ${selectedYear}: ${daysInMonth}`);
          ...
          return (
            <div key={rowIndex} className=" grid grid-cols-7 gap-1">
              {dayList.map((dayOfWeek, weekDayIndex) => {
                ...
                return (
                    <div style={{ background: color }} className={'text-xs sm:text-sm border border-solid p-2 flex items-center gap-2 justify-between rounded-lg ' + (isToday ? ' border-indigo-400' : ' border-indigo-100') + (color === 'white' ? ' text-indigo-400' : ' text-white')} key={weekDayIndex}>
                        <p>{dayIndex}</p>
                    </div>
                )
              })}
